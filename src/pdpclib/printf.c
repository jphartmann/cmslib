/* printf family from stdio                                          */
/*                                John Hartmann 17 Jun 2011 08:12:52 */

/*********************************************************************/
/* Note  that  gcc 4.6.0 implements va_arg as a pointer.  Thus there */
/* is no need to pass the address of it.  We also very much need the */
/* gcc version of stdarg.h.                                          */
/*                                                                   */
/* Change activity:                                                  */
/*17 Jun 2011  +++ Add "n" variants of sprintf.                      */
...
#define outch(ch) ((fq == NULL)                                        \
      ? (0 < (blength) ? ((blength)--, *s++ = (ch)) : ch)              \
      : putc(ch, fq))
/* The  result of this macro is never used.  And it is wrong anyhow, */
/* since modern gcc faults it with righthand has no effect           */
#define outstr(str) ( chcount += len = strlen(str) ,                   \
   (fq ? fputs(str, fq) :                                              \
      (0 < blength ?                                                   \
         (memcpy(s, str, (len <= blength ? len : blength)),            \
            blength-=len, s+=len, 0) : 0)                                 \
   ))
...
static int examine(const char **formt, FILE *fq, char **ps, va_list arg,
                   int chcount, size_t * pblength);
static int vvprintf(const char *format, va_list arg, FILE *fq, char *s, size_t blength);
...
__PDPCLIB_API__ int fprintf(FILE *stream, const char *format, ...)
{
    ...
    #if 0
    __WERROR("fprintf fd %d %p %s", fileno(stream), stream, format);
    #endif
    ...
    while (*format)
    {
        if (*format == '%')
        {
            ...
            else
            {
                ...
                extraCh = examine(&format, fq, &s, arg, chcount, &blength);
                chcount += extraCh;
            }
        }
        else
...
static int
examine(const char **formt, FILE *fq, char **ps, va_list arg,
                   int chcount, size_t * pblength)
{
    int extraCh = 0;
    int flagMinus = 0;
    ...
    else fprintf(stderr, "Library detected error:  Format character '%c' (0x%02x) not recognised.\n", *format, 0xff & *format);
    *formt = format;
    return (extraCh);
}
...
static void
dblcvt(double num, char cnvtype, size_t nwidth,
            int nprecision, char *result)
{
    double b,round;
